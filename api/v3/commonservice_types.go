...
// func (r *CommonService) SetUpdatingCondition(name string, rt ResourceType, cs corev1.ConditionStatus, mu sync.Locker) {
// 	mu.Lock()
// 	defer mu.Unlock()
// 	c := newCondition(ConditionTypePending, cs, "Updating "+string(rt), "Updating "+string(rt)+" "+name)
// 	r.setCondition(*c)
// }

// SetDeletingCondition creates a deleting condition status
// func (r *CommonService) SetDeletingCondition(name string, rt ResourceType, cs corev1.ConditionStatus, mu sync.Locker) {
// 	mu.Lock()
// 	defer mu.Unlock()
// 	c := newCondition(ConditionTypePending, cs, "Deleting "+string(rt), "Deleting "+string(rt)+" "+name)
// 	r.setCondition(*c)
// }
